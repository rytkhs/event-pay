...
export const MobileResponsiveLayout: React.FC<MobileResponsiveLayoutProps> = ({
  ...
  const { deviceType, orientation, breakpoint, isMobile, isTablet, isDesktop } = useViewportSize();
  ...
  const enhanceChild = useCallback((child: React.ReactElement, index: number) => {
    const childProps: any = { ...child.props };
    
    // ブレークポイント関連のクラスを子要素に追加
    if (child.props['data-testid']) {
      const childTestId = child.props['data-testid'];
      if (childTestId.includes('content') || childTestId.includes('container') || childTestId.includes('flex') || childTestId.includes('grid')) {
  ...
  const clonedChildren = useMemo(() => {
    return React.Children.map(children, (child, index) => {
      if (React.isValidElement(child)) {
        return enhanceChild(child, index);
      }
      return child;
    });
