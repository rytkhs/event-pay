...
export function generateWebSiteSchema(): WithContext<WebSite> {
  ...
  return {
    ...
    url: appUrl,
    description:
      "出欠確認から集金まで、リンク1本でまとめて管理できるイベント出欠管理・集金アプリです。参加者はアカウント登録不要で、オンライン支払いと現金払いの両方に対応しています。",
    inLanguage: "ja",
    publisher: {
      "@type": "Organization",
      "@id": `${appUrl}#organization`,
...
export function generateSoftwareApplicationSchema(): WithContext<SoftwareApplication> {
  ...
  return {
    ...
    featureList: [
      ...
      "ハイブリッド決済対応: クレジットカードなどのキャッシュレス決済も現金払いも一つのリストで一元管理。オンライン支払いは自動で入金確認、現金集金も受領ボタンで簡単管理",
