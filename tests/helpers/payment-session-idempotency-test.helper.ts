...
export class PaymentSessionIdempotencyTestHelper {
  ...
  static async createCompleteSetup(
    ...
    const event = await createPaidTestEvent(user.id, {
      title: `${scenarioName}決済冪等性テストイベント`,
      fee: 1000,
    });
    ...
    const adminClient = await factory.createAuditedAdminClient(
      ...
      "Payment session idempotency test setup",
      {
        ...
        additionalInfo: { testContext: "payment-session-idempotency" },
      }
    );

    // 5. 共通の createStripeSession パラメータを作成
  ...
  async cleanup(): Promise<void> {
    await cleanupTestPaymentData({
      attendanceIds: [this.setup.attendance.id],
      eventIds: [this.setup.event.id],
      userIds: [this.setup.user.id],
    });
  }
}
