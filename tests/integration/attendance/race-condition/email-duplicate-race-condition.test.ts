/**
 * TC-RC-002: メール重複レースコンディション対応テスト
 *
...
describe("TC-RC-002: メール重複レースコンディション対応テスト", () => {
  ...
  it("同一メールアドレスで2名が同時回答 → 1名のみ成功すること（無料イベント）", async () => {
    ...
    const errorVerification = ConcurrentRequestHelper.verifyExpectedErrors(
      ...
      "このメールアドレスは既に回答済みです。" // 仕様書で定義されたメッセージ
    );
  ...
  it("同一メールアドレスで3名が同時回答 → 1名のみ成功すること（仕様書補強）", async () => {
  ...
  it("同一メールアドレスで有料イベントに同時回答 → 1名のみ成功し決済も整合すること", async () => {
  ...
  it("異なるメールアドレスでは同時回答が可能なこと（対照テスト）", async () => {
    // 異なるメールアドレスでの参加データ
    const participants: ParticipationFormData[] = [
      {
        inviteToken: testData.freeEvent.invite_token,
