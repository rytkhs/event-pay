...
export class ApiResponseHelper {
  ...
  static validation(error: string | z.ZodError, status = 400): NextResponse {
  ...
  static rateLimit(error: string, retryAfter?: number, status = 429): NextResponse {
    const response: ApiErrorResponse = {
      success: false,
      error,
  ...
  static unauthorized(error = "Unauthorized", code = "UNAUTHORIZED"): NextResponse {
    return this.error(error, code, 401);
  }

  static badRequest(error = "Bad Request", code = "BAD_REQUEST"): NextResponse {
    return this.error(error, code, 400);
  }

  static internalError(error = "Internal Server Error", code = "INTERNAL_ERROR"): NextResponse {
