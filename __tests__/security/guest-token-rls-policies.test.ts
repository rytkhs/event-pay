...
describe("ゲストトークンRLSポリシーテスト", () => {
  ...
  describe("RLSポリシーの基本動作テスト", () => {
    it("有効なゲストトークンで参加記録を読み取りできる", async () => {
      ...
      const { error: setError } = await guestClient.rpc("set_test_guest_token", {
        token: validGuestToken,
      });
  ...
  describe("eventsテーブルのRLSポリシーテスト", () => {
    it("ゲストトークンで参加するイベントの詳細を読み取りできる", async () => {
      ...
      const { data, error } = await guestClient.from("events").select("*").eq("id", testEventId);
    ...
    it("参加していないイベントの詳細は読み取りできない", async () => {
      ...
      const { data, error } = await guestClient.from("events").select("*").eq("id", otherEvent.id);
  ...
  describe("パフォーマンステスト", () => {
    it("インデックスが適切に使用されることを確認", async () => {
      ...
      for (let i = 0; i < 10; i++) {
        await guestClient.from("attendances").select("*").eq("guest_token", validGuestToken);
      }
