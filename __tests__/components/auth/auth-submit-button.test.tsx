...
describe("AuthSubmitButton 改善版", () => {
  ...
  describe("新機能: キャンセル機能", () => {
    ...
    it("キャンセルボタンクリックで正しくハンドラーが呼ばれる", async () => {
      const onCancel = jest.fn();
      render(
        <AuthSubmitButton isPending={true} canCancel={true} onCancel={onCancel}>
          送信
    ...
    it("キャンセル不可能な場合はキャンセルボタンが表示されない", () => {
      const onCancel = jest.fn();
      render(
  ...
  describe("新機能: タイムアウト表示", () => {
    it("タイムアウトが設定されている場合にカウントダウンが表示される", async () => {
      jest.useFakeTimers();
    ...
    it("タイムアウト発生時に正しくハンドラーが呼ばれる", async () => {
      jest.useFakeTimers();
  ...
  describe("エラーハンドリング", () => {
    ...
    it("onCancelが未定義でもキャンセル機能が正常に動作する", () => {
      render(
