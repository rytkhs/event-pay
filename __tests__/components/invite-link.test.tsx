...
describe("InviteLink Component", () => {
  ...
  it("should generate new invite token when button is clicked", async () => {
    ...
    await waitFor(() => {
      expect(mockGenerateInviteTokenAction).toHaveBeenCalledWith("test-event-id", { forceRegenerate: false });
    });
  ...
  describe("Force Regeneration Feature", () => {
    ...
    it("should regenerate token when user confirms", async () => {
      ...
      await waitFor(() => {
        expect(mockGenerateInviteTokenAction).toHaveBeenCalledWith(
          "test-event-id", 
    ...
    it("should display security warning", () => {
      ...
      expect(screen.getByText(/⚠️ リンクが漏洩した場合は「再生成」ボタンで新しいリンクを作成してください。/)).toBeInTheDocument();
    });
  });
});