...
describe("useFocusManagement Hook", () => {
  ...
  test("focusFirstErrorが呼ばれたとき、最初のエラーフィールドにフォーカスすること", () => {
    // エラーフィールドの準備
  ...
  test("trapFocusが有効なとき、モーダル内でフォーカストラップを管理すること", () => {
    // モーダル要素の準備
  ...
  test("restoreFocusが呼ばれたとき、前の要素にフォーカスを復元すること", () => {
  ...
  test("複数のエラーフィールドを正しい優先順位で処理すること", () => {
  ...
  test("フォーカストラップのクリーンアップ関数を提供すること", () => {
    const modal = document.createElement("div");
