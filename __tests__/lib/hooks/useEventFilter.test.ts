...
describe('useEventFilter Hook', () => {
  test('フックが初期状態を正しく返す', () => {
    const { result } = renderHook(() => useEventFilter({ events: mockEvents, enableClientSideFiltering: true }));
  ...
  test('日付範囲フィルターが正しく適用される', () => {
    ...
    expect(result.current.filteredEvents.find(e => e.title === 'キャンセルイベント')).toBeDefined();
  ...
  test('無効なステータスフィルターが渡された場合、デフォルト値にセットされる', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('無効なステータスフィルターです。全件表示に設定します。');
    expect(result.current.filters.status).toBe('all');
  ...
  test('不正な日付範囲が渡された場合、日付範囲がクリアされる', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('終了日は開始日より前の日付は指定できません。日付範囲をクリアします。');
    expect(result.current.filters.dateRange).toEqual({});
  ...
  test('空のイベント配列でも正しく動作する', () => {
    const { result } = renderHook(() => useEventFilter({ events: [], enableClientSideFiltering: true }));
