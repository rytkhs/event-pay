...
describe("FocusManager Utility", () => {
  ...
  test("コンテナ内の最初のフォーカス可能要素を見つけること", () => {
  ...
  test("フォーカス可能要素が存在しない場合、空配列を返すこと", () => {
  ...
  test("セレクターで最初のエラーフィールドにフォーカスすること", () => {
  ...
  test("Shift+Tabでの逆方向フォーカストラップが動作すること", () => {
    const firstInput = createMockElement("first");
    const lastInput = createMockElement("last");
