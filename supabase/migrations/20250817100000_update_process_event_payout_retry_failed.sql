-- ====================================================================
-- process_event_payout: failed レコード再試行対応
-- ====================================================================

-- 本マイグレーションでは、既存の process_event_payout 関数を置き換え
--  failed 状態の payout レコードを再利用出来るように拡張する。
--  具体的には既存 failed 行を pending にリセットし、その ID を返す。
--
-- セキュリティ・アドバイザリロックの仕様は変更しない。
-- ====================================================================
...
COMMENT ON FUNCTION process_event_payout(UUID, UUID) IS 'イベントの送金処理を実行する関数（failed レコードも再利用版）';
