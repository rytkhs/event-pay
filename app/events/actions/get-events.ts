...
export async function getEventsAction(options: GetEventsOptions = {}): Promise<GetEventsResult> {
  try {
    ...
    if (sortBy && !['date', 'created_at', 'attendances_count', 'fee'].includes(sortBy)) {
      return {
        success: false,
        error: "sortByは'date', 'created_at', 'attendances_count', 'fee'のいずれかである必要があります",
      };
    }

    if (statusFilter && !['all', 'upcoming', 'ongoing', 'past', 'cancelled'].includes(statusFilter)) {
      return {
        success: false,
        error: "statusFilterは'all', 'upcoming', 'ongoing', 'past', 'cancelled'のいずれかである必要があります",
      };
    }
    ...
    if (dateFilter && Object.keys(dateFilter).length > 0) {
      ...
      if (!dateValidation.success) {
        const errorMessage = dateValidation.error.errors[0]?.message || "日付フィルターの形式が正しくありません";
        return {
          success: false,
          error: errorMessage,
        };
    ...
    let eventsQuery = applyFilters(supabase.from("events").select(`
      id,
      title,
      date,
      location,
