...
## 概要
本ドキュメントは、招待リンクからゲストが出欠（RSVP）を回答し、必要に応じて決済方法を選択し、後から変更するまでの処理フローを説明する。
スコープ: 招待リンク検証 → 初回RSVP回答 → ゲストトークンURLでの更新（オンライン決済の確定/Webhook処理は別ドキュメント）。
...
## 登場人物と前提
- Guest: 招待リンク（`invite_token`）経由でイベントを閲覧し、RSVP回答するユーザー（アカウント不要）。
...
3. System は回答可能かを判定する（キャンセル済み、締切超過、過去イベント等）。
...
8. System は回答完了画面を返し、ゲストトークンURL（例: `/guest/{guest_token}`）を案内する。
9. イベントに `community.legal_slug` がある場合、ゲスト画面から特商法ページ（`/tokushoho/{legal_slug}`）へ遷移できる。
10. 可能なら回答完了通知メールを送信します（失敗しても回答自体は成功扱いにする、など運用方針に従う）。
...
- 回答期限超過・イベント終了・キャンセル済み → 回答不可の理由を表示。
- 定員超過（attending のみ） → 「定員に達している」エラー。
- 同一イベント内でメール重複 → 「既に回答済み」エラー。
