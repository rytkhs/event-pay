...
- どこまでが「副作用を持つ内部アプリケーションサービス」で、どこまでが「pure helper」なのかが曖昧になり、`AppResult` の適用範囲が広がり過ぎやすい
...
1. **副作用を持つ内部アプリケーションサービスの正規形は `AppResult`**
   - 対象は use case / service / command / DB・外部API・認可を伴う orchestration
   - 期待される失敗は `AppResult` の `success: false` で表現する
   - 失敗は必ず `AppError` を保持する

2. **pure helper は生値を返す**
   - mapper / formatter / selector / builder など、失敗契約を持たない純粋処理は `AppResult` にしない
   - 想定外のみ `throw` する

3. **Server Actions（UI境界）の返却は原則 `ActionResult<T>`**
   - redirect-only は ADR-0014 の条件を満たす場合のみ例外
   - `AppResult <-> ActionResult` の変換は adapter を用い、境界で投影する

4. **HTTP（Route Handlers）の error は RFC 7807 Problem Details**
   - `AppError -> Problem Details` は adapter に集約する
   - success は `{ success: true }` 等のラッパを返さず、payloadのみ（または `204 No Content`）

5. **Server Component / Page / Layout の画面制御は Result 契約とは別扱い**
   - `redirect()` / `notFound()` は UI routing 制御として許容する
   - ただし、内部アプリケーションサービス自体を redirect-only にしない
...
- `AppResult` を使う場所と使わない場所が責務ベースで明確になる
