...
### コミュニティ運営
- コミュニティ作成・切り替え（現在選択中コミュニティを軸にしたワークスペース型UI）。
- コミュニティ設定の確認・編集・削除。
- コミュニティ公開ページ、問い合わせフォーム、特商法ページの提供。

### イベント管理
- 現在選択中コミュニティ配下でのイベント作成・管理。
- 支払い設定（無料 / 現金のみ / オンライン決済 / 両方）。
- 出欠回答期限・支払い期限・猶予期間などの期限管理。
...
- コミュニティに紐づく payout profile を基準にした受取先管理。
- 冪等性を意識した決済リトライ設計。
...
│   ├── (app)/               # ログイン後アプリ（ダッシュボード / イベント / 設定）
│   ├── (auth)/              # 認証関連ページ
│   ├── (focus)/             # コミュニティ作成などの集中導線
│   ├── (marketing)/         # ランディング
│   ├── (public)/            # コミュニティ公開ページ / 法務ページ
